...
$data_receta = "SELECT p.nombre,rc.tipo,rc.total,rc.fecha FROM `resumen_compra` rc, proveedores p  WHERE rc.proveedor=p.id AND rc.fecha BETWEEN '$fecha_desde 00:00:00' AND '$fecha_hasta 23:59:59'";
$ejecutar_receta = $conexion->query($data_receta);

$totsql = "SELECT SUM(total) FROM `resumen_compra` WHERE fecha BETWEEN '$fecha_desde 00:00:00' AND '$fecha_hasta 23:59:59'";
...
$pdf->Cell(0,0, 'Reporte de Compras desde: '.$fecha_desde.' Hasta: '.$fecha_hasta,0,1,'C');
$pdf->Ln(5);
...
$pdf->Cell(80,6,'Proveedor',1,0,'C',1);
...
$cont=1;
 while($row = $ejecutar_receta->fetch_assoc())
 {
    ...
    $pdf->Cell(80,6,utf8_decode($row['nombre']),1,0,'');
    $pdf->Cell(20,6,utf8_decode($retVal = ($row['tipo']==1) ? "Contado" : "Credito"),1,0,'');
